...
var genericPackage = turbocache.Package{
	PackageInternal: turbocache.PackageInternal{
		Name:    "something",
		Type:    "generic",
		Sources: []string{"*.txt"},
	},
	Config: turbocache.GenericPkgConfig{
...
func TestScriptArgs(t *testing.T) {
	...
	tests := []*testutil.CommandFixtureTest{
		// If the argument isn't passed then Turbocache should fail with an exit code of 1
		{
			Name:              "unresolved arg",
			T:                 t,
...
func TestWorkingDirLayout(t *testing.T) {
	...
	setup := &testutil.Setup{
		Components: []testutil.Component{
			{
				Location: "scripts",
				Packages: []turbocache.Package{genericPackage},
				Scripts: []turbocache.Script{
					{
						Name:          "pwd-origin",
...
func TestScriptParallel(t *testing.T) {
	...
	tests := []*testutil.CommandFixtureTest{
		...
		// When one of the scripts fail, Turbocache should still run the other scripts to completions
