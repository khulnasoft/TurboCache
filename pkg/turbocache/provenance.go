...
const (
	...
	ProvenanceBuilderID = "github.com/khulnasoft/turbocache"
)
...
func (p *Package) produceSLSAEnvelope(buildctx *buildContext, subjects []in_toto.Subject, buildStarted time.Time) (res *provenance.Envelope, err error) {
	...
	pred.Builder = in_toto.ProvenanceBuilder{
		ID: fmt.Sprintf("%s:%s@sha256:%s", ProvenanceBuilderID, Version, buildctx.turbocacheHash),
	}
	...
	pred.Recipe = in_toto.ProvenanceRecipe{
		Type:              fmt.Sprintf("https://github.com/khulnasoft/turbocache/build@%s:%d", p.Type, buildProcessVersions[p.Type]),
		Arguments:         os.Args,
