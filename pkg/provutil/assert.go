...
var AssertBuiltWithTurbocache = &Assertion{
	Name:        "built-with-turbocache",
	Description: "ensures all bundle entries have been built with turbocache",
	Run: func(stmt *provenance.Statement) []Violation {
		if strings.HasPrefix(stmt.Predicate.Builder.ID, turbocache.ProvenanceBuilderID) {
			return nil
		}

		return []Violation{
			{Desc: "was not built using turbocache"},
		}
	},
}

func AssertBuiltWithTurbocacheVersion(version string) *Assertion {
	return &Assertion{
		Name:        "built-with-turbocache-version",
		Description: "ensures all bundle entries which have been built using turbocache, used version " + version,
		Run: func(stmt *provenance.Statement) []Violation {
			if !strings.HasPrefix(stmt.Predicate.Builder.ID, turbocache.ProvenanceBuilderID) {
				return nil
			}

			if stmt.Predicate.Builder.ID != turbocache.ProvenanceBuilderID+":"+version {
				return []Violation{{Desc: "was built using turbocache version " + strings.TrimPrefix(stmt.Predicate.Builder.ID, turbocache.ProvenanceBuilderID+":")}}
			}
