...
func init() {
	register(PackageCheck("copy-from-pacakge", "attempts to find broken package paths in COPY and ADD statements", turbocache.DockerPackage, checkDockerCopyFromPackage))
}
...
func checkDockerCopyFromPackage(pkg *turbocache.Package) ([]Finding, error) {
	cfg, ok := pkg.Config.(turbocache.DockerPkgConfig)
	if !ok {
		// this is an error as compared to a finding because the issue most likely is with turbocache,
