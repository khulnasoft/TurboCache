...
func init() {
	register(PackageCheck("use-package", "attempts to find broken package paths in the commands", turbocache.GenericPackage, checkArgsReferingToPackage))
}

func checkArgsReferingToPackage(pkg *turbocache.Package) ([]Finding, error) {
	cfg, ok := pkg.Config.(turbocache.GenericPkgConfig)
	if !ok {
