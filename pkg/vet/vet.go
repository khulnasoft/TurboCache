...
type checkFunc struct {
	...
	runPkg func(pkg *turbocache.Package) ([]Finding, error)
	runCmp func(pkg *turbocache.Component) ([]Finding, error)
}
...
func (cf *checkFunc) Init(turbocache.Workspace) error {
	return nil
}

func (cf *checkFunc) RunPkg(pkg *turbocache.Package) ([]Finding, error) {
	if cf.runPkg == nil {
...
func (cf *checkFunc) RunCmp(pkg *turbocache.Component) ([]Finding, error) {
	if cf.runCmp == nil {
...
// PackageCheck produces a new check for a turbocache package
func PackageCheck(name, desc string, tpe turbocache.PackageType, chk func(pkg *turbocache.Package) ([]Finding, error)) Check {
	return &checkFunc{
		info: CheckInfo{
...
// ComponentCheck produces a new check for a turbocache component
func ComponentCheck(name, desc string, chk func(pkg *turbocache.Component) ([]Finding, error)) Check {
	return &checkFunc{
		info: CheckInfo{
...
type Check interface {
	...
	Init(ws turbocache.Workspace) error
	RunPkg(pkg *turbocache.Package) ([]Finding, error)
	RunCmp(pkg *turbocache.Component) ([]Finding, error)
}
...
type CheckInfo struct {
	...
	Description   string
	PackageCheck  bool
	AppliesToType *turbocache.PackageType
}

// Finding describes a check finding. If the package is nil, the finding applies to the component
type Finding struct {
	Check       string
	Component   *turbocache.Component
	Package     *turbocache.Package
	Description string
	Error       bool
}
...
func Run(workspace turbocache.Workspace, options ...RunOpt) ([]Finding, []error) {
	var opts runOptions
	...
	var (
		...
		runCompCheck = func(c Check, comp *turbocache.Component) {
			info := c.Info()
		...
		runPkgCheck = func(c Check, pkg *turbocache.Package) {
			info := c.Info()
			if !info.PackageCheck {
				return
			}
