...
func init() {
	...
	register(PackageCheck("node-modules-source", "checks if the package's sources include node_modules/", turbocache.YarnPackage, checkYarnNodeModulesSource))
}
...
func (c *checkImplicitTransitiveDependencies) Init(ws turbocache.Workspace) error {
	c.pkgs = make(map[string][]string)
	for pn, p := range ws.Packages {
		if p.Type != turbocache.YarnPackage {
			continue
		}
...
func (c *checkImplicitTransitiveDependencies) getPkgJSON(pkg *turbocache.Package) (*pkgJSON, error) {
	var (
		found bool
...
func (c *checkImplicitTransitiveDependencies) RunCmp(pkg *turbocache.Component) ([]Finding, error) {
	return nil, fmt.Errorf("not a component check")
}

func (c *checkImplicitTransitiveDependencies) RunPkg(pkg *turbocache.Package) ([]Finding, error) {
	...
	for yarnDep, src := range depsInCode {
		var found bool
		for _, turbocacheDep := range c.pkgs[yarnDep] {
			for _, dep := range pkg.GetDependencies() {
				if dep.FullName() == turbocacheDep {
					found = true
					break
				}
			}
...
func checkYarnNodeModulesSource(pkg *turbocache.Package) ([]Finding, error) {
	var res []string
	for _, src := range pkg.Sources {
