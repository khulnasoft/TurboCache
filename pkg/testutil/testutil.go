...
type Setup struct {
	Workspace  turbocache.Workspace `yaml:"workspace"`
...
type Component struct {
	...
	Files    map[string]string    `yaml:"files"`
	Comp     turbocache.Component `yaml:"comp"`
	Packages []turbocache.Package `yaml:"packages"`
	Scripts  []turbocache.Script  `yaml:"scripts"`
}
...
// Materialize produces a turbocache workspace according to the setup spec
func (s Setup) Materialize() (workspaceRoot string, err error) {
	workspaceRoot, err = os.MkdirTemp("", "turbocache-test-*")
	if err != nil {
		return
	}
	...
	for _, comp := range s.Components {
		...
		cmp := struct {
			Constants turbocache.Arguments `yaml:"const,omitempty"`
			Packages  []turbocache.Package `yaml:"packages,omitempty"`
			Scripts   []turbocache.Script  `yaml:"scripts,omitempty"`
		}{
...
func (ft *CommandFixtureTest) Run() {
	...
	ft.T.Run(ft.Name, func(t *testing.T) {
		...
		n := 0
		for _, x := range env {
			if strings.HasPrefix(x, "TURBOCACHE_") {
				continue
			}
			env[n] = x
			n++
