...
For each matching package turbocache will execute the specified command in the package component's origin.
...
Example use:
  ...
  turbocache exec --filter-type yarn -- pwd

  # run go get in all Go packages
  turbocache exec --filter-type go -- go get -v ./...

  # list all Go packages in the workspace
  turbocache exec --filter-type go --raw-output -- echo {}

  # execute go build in all direct Go dependencies when any of the relevant source files changes:
  turbocache exec --package some/other:package --dependencies --filter-type go --parallel --watch -- go build

  # run tsc watch for all dependent yarn packages (once per component origin):
  turbocache exec --package some/other:package --transitive-dependencies --filter-type yarn --parallel -- tsc -w --preserveWatchOutput
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		var pkgs map[*turbocache.Package]struct{}
		if len(packages) == 0 {
			pkgs = make(map[*turbocache.Package]struct{}, len(ws.Packages))
			for _, p := range ws.Packages {
				pkgs[p] = struct{}{}
			}
		} else {
			pkgs = make(map[*turbocache.Package]struct{}, len(packages))
		...
		spkgs := make([]*turbocache.Package, 0, len(pkgs))
		for p := range pkgs {
			spkgs = append(spkgs, p)
		}
		turbocache.TopologicalSort(spkgs)
		...
		if watch {
			...
			evt, errs := turbocache.WatchSources(context.Background(), spkgs, 2*time.Second)
			for {
				select {
				case <-evt:
					err := executeCommandInLocations(args, locs, noExecCache{}, parallelism, rawOutput)
...
func (c filesystemExecCache) filename(loc commandExecLocation) (string, error) {
	...
	if loc.Package != nil {
		...
		id = turbocache.FilesystemSafeName(loc.Component.Name)
	} else if loc.Dir != "" {
		id = turbocache.FilesystemSafeName(loc.Dir)
	} else if loc.Name != "" {
		id = loc.Name
	}
