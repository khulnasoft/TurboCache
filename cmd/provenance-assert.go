...
var provenanceAssertCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if signed, err := cmd.Flags().GetBool("signed"); err != nil {
			...
			var keyPath string
			if pkg == nil {
				keyPath = os.Getenv("TURBOCACHE_PROVENANCE_KEYPATH")
			} else {
				keyPath = pkg.C.W.Provenance.KeyPath
			}
			if keyPath == "" {
				log.Fatal("no key path specified - use the TURBOCACHE_PROVENANCE_KEYPATH to specify one")
			}
		...
		if do, err := cmd.Flags().GetBool("built-with-turbocache"); err != nil {
			log.Fatal(err)
		} else if do {
			assertions = append(assertions, provutil.AssertBuiltWithTurbocache)
		}
		if ver, err := cmd.Flags().GetString("built-with-turbocache-version"); err != nil {
			log.Fatal(err)
		} else if ver != "" {
			assertions = append(assertions, provutil.AssertBuiltWithTurbocacheVersion(ver))
		}
		...
		if pkg == nil {
			...
			err = turbocache.AccessAttestationBundleInCachedArchive(pkgFN, func(bundle io.Reader) error {
...
func getProvenanceTarget(cmd *cobra.Command, args []string) (bundleFN, pkgFN string, pkg *turbocache.Package, err error) {
...
func init() {
	...
	provenanceAssertCmd.Flags().Bool("built-with-turbocache", false, "ensure that all entries in the attestation bundle are built by turbocache")
	provenanceAssertCmd.Flags().String("built-with-turbocache-version", "", "ensure that all entries in the attestation bundle are built by a specific turbocache version")
